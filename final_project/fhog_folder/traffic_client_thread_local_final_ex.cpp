...
void* img_recv(void*arg)
{
	...
	string videoStreamAddress = "rtsp://192.168.0.30:8554/test";	
...
void create_color_box(Mat* (&Color), Point*(&array_cpt_tl), int size)
{
	for(int i=0;i<size;i++)
	{
		...
		if(array_cpt_tl[i*2+1].x < 0)
		{
			array_cpt_tl[i*2+1].x = 0;
		}

		if(array_cpt_tl[i*2].x > img.cols)
		{
			array_cpt_tl[i*2].x = img.cols;
		}
		...
		if(array_cpt_tl[i*2+1].y < 0)
		{
			array_cpt_tl[i*2+1].y = 0;
		}

		if(array_cpt_tl[i*2].y > img.rows)
		{
			array_cpt_tl[i*2].y = img.rows;
		}
...
int hsv_handler(Mat &img)
{
	...
	if(red_positive == 0 && green_positive > 0)
		return 2;
	else if(green_positive == 0 && red_positive > 0)
		return 1;
	else
		return 0;
}
