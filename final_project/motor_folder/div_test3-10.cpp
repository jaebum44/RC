...
int motor_ctrl(float sl_servo, float sl_min, int cols)
{
	...
	{
		if( sl_min > 0 && sl_servo < cols*15/16)
		{

			if(sl_servo < cols*7/16)
			{
				printf("turn left \n");
				servo=160;
				dc_motor*= 1.3;
			}
			else if(sl_servo < cols*11/16)
			{
		...
		else if( sl_min < 0 &&  sl_servo > cols*1/16) 
		{

			if(sl_servo > cols*9/16)
			{
				servo=320;
				printf("turn right %d\n",servo);
				dc_motor*= 1.3;
			}
			else if(sl_servo > cols*5/16)
			{
