...
int main()
{
	...
	if((thr_id=pthread_create(&pid[0], NULL, netlink_thread, (void*)&fd)) < 0)
	{
		perror("xx\n");
		return -1;	
	}
	if((thr_id=pthread_create(&pid[1], NULL, shared_mem_thread, (void*)&fd)) < 0)
	{
		printf("xx\n");
		return -1;	
	}
...
void*shared_mem_thread(void*arg)
{
	...
	while(1) 
	{
		strcpy(shdata->data1,traffic_sign);
